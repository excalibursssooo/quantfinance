...
你是一位精通华尔街各类估值模型的资深分析师。
请根据以下 {ticker} 的财务数据，决定最合适的估值模型，并**调用对应的工具**进行计算。
...
- 自由现金流 (FCF): {fcf}
- EBITDA: {ebitda}
- 净债务 (Net Debt): {net_debt}
- 当前市销率 (P/S): {ps_ratio}
- 当前 EV/EBITDA: {ev_ebitda}

【你的任务】:
1. 判定路径：FCF为正用DCF；FCF为负/高增长科技用P/S；重资产用EV/EBITDA。
2. 推算所需的参数（例如：依据历史设定合理的 target_ps 或 g）。
3. **强制要求：你必须调用计算工具，并将你的逻辑填入 reasoning 参数中。**
""")