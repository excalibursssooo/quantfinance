...
def generate_dynamic_query(ticker: str, concerns: str, horizon: str) -> str:
    """内部辅助函数：使用极小参数的 LLM 动态生成搜索词"""
    llm = Config.get_llm(temperature=0.1).with_structured_output(SearchQuery)
...
def get_real_market_data(ticker: str, concerns: str = "general outlook", horizon: str = "long-term") -> str:
    """
    智能重构后的真实市场数据抓取工具
    """
    ...
    # 1. 动态生成 Query
    smart_query = generate_dynamic_query(ticker, concerns, horizon)
